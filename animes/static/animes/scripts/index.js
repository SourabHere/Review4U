...
function updatePopover() {
    ...
    Array.from(more).forEach(element => {
        let Aname = element.textContent;

        curclass = document.querySelector(`.A${count}`);
        originnalname = Aname;
        ...
        Animeimg = document.getElementById(`Anime${count}`);
        ...
        popstr = popstr + `<div class="card mb-3 cardrecc box${count}" style="width:auto;">
                <div class="row g-0 content${count} poparea">
                    <div class="col-md-4 popimgarea">
                        <a href="${detail.getAttribute("href")}" class="linkimg"><img src="${Animeimg.getAttribute("src")}" class="img-fluid rounded-start popimage" alt="..."></a>
                        </div>
                        <div class="col-md-8 poprecc">
...
search.addEventListener("input", function () {
    ...
    Array.from(searchpop).forEach(element => {
        ...
        let Animehere = cardn.textContent;
        Animehere = Animehere.toLowerCase();

        // console.log(Animehere);
        // console.log(count);

        if (Animehere == searchval || Animehere.slice(0, searchval.length) == searchval) {
            box.style.display = "";
        }
        else {
...
searchbtn.addEventListener("click", function (event) {
    ...
    Array.from(more).forEach(element => {
        ...
        // console.log(finalinp);
        // console.log(Sname);
        // console.log(finalinp==(Sname));

        if (Aname == finalinp || Aname.slice(0, finalinpsize) == finalinp) {
            // console.log("yahooo");
            curclass.style.display = "";
        }
        // else if (Aname != finalinp) {
        //     console.log((Aname.slice(0, 2)));
        //     if ((Aname.slice(0, 2)).includes(finalinp)) {
        //         curclass.style.display = "";
        //     }
        //     else {
        ...
        //     }
        // }
        else if ((Aname.includes(finalinp)) && (finalinp.length > 4)) {
            curclass.style.display = "";
        }
        else {
            // console.log("nopes");
